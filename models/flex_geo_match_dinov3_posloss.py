...
POS_REG_BETA = 0.1
POS_REG_LOSS = "l2_sum"
POS_HEATMAP_SIGMA = 1.0
POS_HEATMAP_LOSS = "soft_ce"
POS_HEATMAP_XY_WEIGHT = 0.25

def infer_pos_heatmap_hw(num_bins: int, pos_mode:str, pos_grid:int) -> Tuple[int, int]:
    bins = int(num_bins)
    if bins <= 0:
...
def pos_logits_to_heatmap(
    ...
    if pos_logits.shape[-1] == 2:
        ...
        H, W = infer_pos_heatmap_hw(
            4 if pos_mode == "quadrant" else max(int(pos_grid), 1) ** 2, 
            pos_mode=pos_mode, 
            pos_grid=pos_grid
        )
    ...
    H, W = infer_pos_heatmap_hw(int(pos_logits.shape[-1]), pos_mode=pos_mode, pos_grid=pos_grid)
...
def compute_pos_reg_loss(
    ...
    if mode_norm == "smooth_l1":
        beta = float(smooth_l1_beta)
        if beta > 0.0:
            return F.smooth_l1_loss(pos_xy_pred[valid], target_xy[valid], beta=beta)
        return F.l1_loss(pos_xy_pred[valid], target_xy[valid])
    if mode_norm == "l1":
        return F.l1_loss(pos_xy_pred[valid], target_xy[valid])

    sq_dist = torch.sum((pos_xy_pred - target_xy) ** 2, dim=-1)  # [B,N]
    if mode_norm == "l2_mean":
        return sq_dist[valid].mean()
    if mode_norm == "l2_sum":
...
def compute_pos_heatmap_loss(
    ...
    heatmap_hw = infer_pos_heatmap_hw(K, pos_mode=pos_mode, pos_grid=pos_grid)
...
def compute_position_loss(
    ...
    if pos_loss_type == "heatmap":
        ...
        pos_loss, _, _ = compute_pos_heatmap_loss(
            ...
            pos_mode=str(getattr(core_model, "pos_mode", "grid")),
            pos_grid=int(getattr(core_model, "pos_grid", 2)),
        )
...
# def infer_pos_head_variant_from_state(state: Dict[str, Any], default: str = "pairwise_residual") -> str:
...
# def infer_separate_pos_neck_from_state(state: Dict[str, Any], default: bool = True) -> bool:
#     if not isinstance(state, dict):
#         return bool(default)
#     keys = set(state.keys())
...
class FlexGeoApprox(nn.Module):
    def __init__(
        ...
        pos_mode: str = "grid",
        pos_grid: int = 4,
        pos_loss_type: str = "reg",
        pos_reg_beta: float = 0.1,
        pos_head_variant: str = "pairwise_residual",
        pos_head_hidden_dim: int = 1024,
        pos_head_depth: int = 2,
        separate_pos_neck: bool = True,
        pretrained: bool = True,
    ...
    def predict_position_outputs(
        ...
        if self.uses_token_pos_head:
            ...
            if self.pos_loss_type == "reg":
                ...
                heat = pos_logits_to_heatmap(pos_logits.reshape(B * N, K), pos_mode=pos_mode, pos_grid=pos_grid)
                out["pos_xy_pred"] = heatmap_to_xy(heat).view(B, N, 2)
            else:
                H_tok, W_tok = sat_pos_token_grid.shape[1:3]
                target_hw = infer_pos_heatmap_hw(
                    4 if self.pos_mode == "quadrant" else int(self.pos_grid) ** 2,
                    pos_mode=pos_mode,
                    pos_grid=pos_grid
                )
...
def train_one_epoch(
    ...
    for step, b in enumerate(loader):
        ...
        with torch.amp.autocast(device_type=amp_device, enabled=use_amp):
            ...
            #                 valid_mask=pos_mask,
            #                 mode=pos_reg_loss,
            #                 smooth_l1_beta=float(getattr(core_model, "pos_reg_beta", 0.1)),
            #             )
            #             if pos_loss is not None:
            #                 loss = loss + pos_weight * pos_loss
            #             else:
        ...
        if main_process and step % max(int(log_every), 1) == 0:
            ...
            msg = (
                ...
                f"ground: {ground.shape} sat:{sat.shape}"
...
def eval_model(
    ...
    # ---- distributed reduction of scalar accumulators ----
    if distributed:
        for k in hits:
    ...
    if total > 0:
        # in-sample branch: hits were accumulated directly
